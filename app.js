class WhiteboardStudio {
    ...
    addImage(src) {
        ...
        this.slides[this.currentSlideIndex].elements.push({ type: 'image', element: img, x, y, width });
    }
    ...
    animatePath(el) {
        return new Promise(resolve => {
            ...
            const duration = (length / 100) / speed * 1000;
    ...
    animateText(el) {
        return new Promise(resolve => {
            ...
            const duration = (bbox.width / 50) / speed * 1000;
            let start = null;
            const step = (timestamp) => {
                if (!start) start = timestamp;
                const progress = timestamp - start;
    ...
    animateImage(el) {
        return new Promise(resolve => {
            ...
            const step = (timestamp) => {
                ...
                const y = el.y + (row / rows) * 150; // 150 is approx height
    ...
    async captureFrame() {
        ...
        currentElements.filter(el => el.type === 'image').forEach(el => {
            if (el.element.style.opacity > 0) {
                ...
                const h = el.element.naturalHeight * (el.width / el.element.naturalWidth);
